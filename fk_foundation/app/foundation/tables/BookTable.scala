...
trait BookTable {
   ...
   *  @param author_id Database column author_id SqlType(BIGINT)
  ...
  case class BookRow(author_id: Long, title: String, published_in: Int, language_id: Int, id: Option[Long])
  /** GetResult implicit for fetching BookRow objects using plain SQL queries */
  implicit def GetResultBookRow(implicit e0: GR[Long], e1: GR[String], e2: GR[Int], e3: GR[Option[Long]]): GR[BookRow] = GR{
    prs => import prs._
    val r = (<<?[Long], <<[Long], <<[String], <<[Int], <<[Int])
    import r._
  ...
  class Book(_tableTag: Tag) extends profile.api.Table[BookRow](_tableTag, Some("mylibrary"), "book") {
    ...
    val author_id: Rep[Long] = column[Long]("author_id")
