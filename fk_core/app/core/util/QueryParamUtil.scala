    ...
    if (queryParamModel.filters.isDefined) {
      queryParamModel.filters.get.foreach(filter => {
        ...
        if (filter.filterComparator.equals("like")) {
          val compare: A => Rep[Boolean] =
            _.queryParamFields(filter.filterName).asInstanceOf[Rep[String]].like(filter.filterValue);
          extendedQuery = extendedQuery.filter(compare);
        } else if (filter.filterComparator.equals("in")) {
          val compare: A => Rep[Boolean] =
            _.queryParamFields(filter.filterName).asInstanceOf[Rep[String]].inSet(filter.filterValue.split(",").toList);
          extendedQuery = extendedQuery.filter(compare);
        }
      })
    }
