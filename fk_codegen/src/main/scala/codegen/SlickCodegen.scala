...
import org.testcontainers.containers.{GenericContainer, MariaDBContainer}
import org.testcontainers.utility.DockerImageName
import play.api.db.evolutions.ThisClassLoaderEvolutionsReader.evolutions
import play.api.db.evolutions.{Evolutions, EvolutionsReader, SimpleEvolutionsReader}
...
class SlickCodegen {
  ...
  def rebuild(clearOld: Boolean) = {
    ...
    val outputDir = "./fk_core/src/main/scala"
    val pkg = "tables"


    // Define the MariaDB test container
    var mariaDBContainer: MariaDBContainer[_] = new MariaDBContainer(DockerImageName.parse("mariadb:10.4.17")).withDatabaseName(databaseName)
    ...
    try {
      val db = slick.jdbc.MySQLProfile.api.Database.forURL(mariaDBContainer.getJdbcUrl, username, password, driver = jdbcDriver)
      ...
      val dbio = MySQLProfile.createModel(Some(MTable.getTables(Some("mylibrary"), None, None, Some(Seq("TABLE", "VIEW")))))
