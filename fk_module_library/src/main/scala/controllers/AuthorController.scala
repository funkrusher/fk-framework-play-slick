...
class AuthorController @Inject()(cc: ControllerComponents, authorManager: AuthorManager)
  ...
  def paginate: Action[AnyContent] = Action.async { implicit request =>
   ...
   */
  def addAuthor = Action.async(parse.json[AuthorDTO]) { implicit request =>
   ...
   */
  def deleteAuthor(authorId: Int) = Action.async { implicit request =>
